...
export function getSubscriptionFromDatabase(
  userId: string
...
export async function getSubscriptionFromDatabase(
  userId: string,
  options: SubscriptionLookupOptions = {}
): Promise<SubscriptionDatabaseRecord | undefined> {
  ...
  try {
    ...
    const gtcUserId = options.gtcUserId?.trim();
    if (gtcUserId) {
      const gtcMatch = records.byGtcId.get(gtcUserId);
      if (gtcMatch) {
        return gtcMatch;
      }
    }
